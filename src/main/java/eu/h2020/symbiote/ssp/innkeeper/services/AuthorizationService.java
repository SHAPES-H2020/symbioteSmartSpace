...
public class AuthorizationService {
     ...
     * @param sspId the sspId
     * @param httpHeaders the headers of the response
     * @return
     */
    public boolean validateServiceResponse(String component, String sspId, HttpHeaders httpHeaders) {
        String serviceResponse = httpHeaders.get(SecurityConstants.SECURITY_RESPONSE_HEADER).get(0);

        if (serviceResponse == null)
            log.warn("There no was service response in the reply of " + component + " ssp " + sspId);


        boolean isServiceResponseVerified;
        try {
            isServiceResponseVerified = componentSecurityHandler.
                    isReceivedServiceResponseVerified(serviceResponse, component, sspId);
        } catch (SecurityHandlerException e) {
        ...
        if (!isServiceResponseVerified) {
            log.warn("The service response in the reply of " + component + " ssp " + sspId + " was not verified");
            return false;
        }
    ...
    private boolean checkSingleLocalHomeTokenAccessPolicy(SecurityRequest securityRequest)
        ...
        requiredClaims.put(Claims.ISSUER, sspId);
